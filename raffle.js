...
function updateWinnersDisplay() {
    ...
    if (state.winners.length === 0) {
        winnersList.innerHTML = '<div class="empty-winners">No winners yet. Start drawing!</div>';
        return;
    }
...
function updatePrizeStatus() {
    ...
    if (state.currentPrize >= state.prizeDescriptions.length) {
        status.textContent = 'All prizes have been drawn!';
    } else {
        status.textContent = `Next: ${state.prizeDescriptions[state.currentPrize]}`;
    }
}
...
function animateSelection(callback) {
    ...
    function animate() {
        ...
        const ticketElement = document.querySelector(`[data-ticket="${randomTicket}"]`);
        if (ticketElement) {
...
function handleDrawClick() {
    ...
    animateSelection((winningTicket) => {
        if (winningTicket === null) {
            ...
            button.textContent = 'Draw Next Prize';
            alert('No more tickets available!');
            return;
        }
        ...
        // Update UI
        const ticketElement = document.querySelector(`[data-ticket="${winningTicket}"]`);
        if (ticketElement) {
        ...
        if (state.currentPrize < state.prizeDescriptions.length) {
            ...
            button.textContent = 'All Prizes Drawn';
        }
    });
}
...
function init() {
    ...
    document.getElementById('tickets-heading').textContent =
        `🎫 Sold Tickets (${state.soldTickets.length})`;

    // Event listeners
    document.getElementById('draw-button').addEventListener('click', handleDrawClick);
